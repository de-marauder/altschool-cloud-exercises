...
# Creating a VPC
echo "creating VPC..."
VPC=$(aws ec2 create-vpc --cidr-block 120.10.0.0/16 --tag-specification 'ResourceType=vpc,Tags=[{Key=Name,Value=altschool-project-vpc}]')
...
# Creating 2 subnets
echo "Creating 2 subnets"
echo ">>==============<<"

echo "Creating subnet 1"
SUBNET1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 120.10.1.0/24 --tag-specification 'ResourceType=subnet,Tags=[{Key=Name,Value=altschool-project-subnet-1-public}]')
...
SUBNET2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 120.10.2.0/24 --tag-specification 'ResourceType=subnet,Tags=[{Key=Name,Value=altschool-project-subnet-2-private}]')
SUBNET2_ID=$(echo $SUBNET2 | jq -r .Subnet.SubnetId)
echo $SUBNET2 | jq -r
echo 'private-subnet-id = ' $SUBNET2_ID
